...
document
  ...
  .addEventListener("click", sendUpdatedFoundData);

function sendUpdatedFoundData() {
  const xhr = new XMLHttpRequest();
  xhr.open("POST", "/foundData", true);
  xhr.setRequestHeader("Content-Type", "application/json");
  let data = JSON.parse(sessionStorage.getItem("foundData"));
  sessionStorage.clear(); // clear session storage
  console.log("Data from screen 3", data);
  const params = {
    title: data.title,
